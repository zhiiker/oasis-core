...
func (p *Point) UnmarshalBinary(data []byte) error {
	...
	if err := checkPoint.isWellFormed(); err != nil {
		return fmt.Errorf("scrape/s11n: deserialized point is invalid: %w", err)
	}
	if checkPoint2, ok := inner.(isCanonicalAble); ok {
		...
		if !checkPoint2.IsCanonical(data) {
			return fmt.Errorf("scrape/s11n: point is not in canonical form")
		}
	}
...
func (p *Point) UnmarshalPEM(data []byte) error {
	...
	if err != nil {
		return fmt.Errorf("scrape/s11n: failed to deserialize PEM encoded point: %w", err)
	}
...
func (p Point) MarshalBinary() ([]byte, error) {
	if err := p.isWellFormed(); err != nil {
		return nil, fmt.Errorf("scrape/s11n: refusing to serialize invalid point: %w", err)
	}
...
func (p *Point) LoadPEM(fn string, scalar *Scalar) error {
	...
	if err != nil {
		if os.IsNotExist(err) && scalar != nil {
			if err = scalar.isWellFormed(); err != nil {
				return fmt.Errorf("scrape/s11n: refusing to use invalid scalar to generate point: %w", err)
			}
	...
	if err != nil {
		return fmt.Errorf("scrape/s11n: failed to read PEM serialized point: %w", err)
	}

	if err = p.UnmarshalPEM(buf); err != nil {
		return fmt.Errorf("scrape/s11n: failed to parse PEM serialized point: %w", err)
	}

	if scalar != nil {
		if err = scalar.isWellFormed(); err != nil {
			return fmt.Errorf("scrape/s11n: invalid scalar provided for verification: %w", err)
		}

		checkPoint := scalar.Point()
		if !p.Inner().Equal(checkPoint.Inner()) {
			return fmt.Errorf("scrape/s11n: point PEM is not for scalar")
		}
	}
...
func (p *Point) isWellFormed() error {
	...
	if !pointIsValid(p.inner) {
		return fmt.Errorf("scrape/s11n: point is invalid")
	}
...
func (s *Scalar) Point() Point {
	if err := s.isWellFormed(); err != nil {
		panic(fmt.Errorf("scrape/s11n: malformed scalar for basepoint multiply: %w", err))
	}
...
func (s *Scalar) UnmarshalPEM(data []byte) error {
	...
	if err != nil {
		return fmt.Errorf("scrape/s11n: failed to deserialize PEM encoded scalar: %w", err)
	}
...
func (s *Scalar) LoadOrGeneratePEM(fn string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			if newScalar, _, err = NewKeyPair(); err != nil {
				return fmt.Errorf("scrape/s11n: failed to generate new scalar: %w", err)
			}
	...
	buf, err := ioutil.ReadAll(f)
	if err != nil {
		return fmt.Errorf("scrape/s11n: failed to read PEM serialized scalar: %w", err)
	}

	if err = s.UnmarshalPEM(buf); err != nil {
		return fmt.Errorf("scrape/s11n: failed to parse PEM serialized scalar: %w", err)
	}
	if err = s.isWellFormed(); err != nil {
		return fmt.Errorf("scrape/s11n: deserialized scalar is invalid: %w", err)
	}
