...
func doTestScrape(t *testing.T, cfg *testScrapeCfg) {
	require := require.New(t)

	instances, publicKeys := initInstances(require, numNodes, protocolThreshold, pvssThreshold)

	verifier, err := New(&Config{
		Participants:  publicKeys,
		Threshold:     protocolThreshold,
		PVSSThreshold: pvssThreshold,
	})
...
func doBenchCommit(b *testing.B, n int) {
	...
	for i := 0; i < b.N; i++ {
		b.StopTimer()
		instances, _ := initInstances(require, n, n, n)
		b.StartTimer()
...
func doBenchOnCommit(b *testing.B, n int) {
	...
	for i := 0; i < b.N; i++ {
		b.StopTimer()
		instances, publicKeys := initInstances(require, n, n, n)
		verifier, err := New(&Config{
			Participants:  publicKeys,
			Threshold:     n,
			PVSSThreshold: n,
...
func initInstances(require *require.Assertions, n, tProto, tPvss int) ([]*Instance, []Point) {
	var (
		instances   []*Instance
		privateKeys []*Scalar
		publicKeys  []Point
