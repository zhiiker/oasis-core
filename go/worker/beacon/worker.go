...
var stateStoreKey = []byte("scrape round state")
...
func (w *Worker) worker() {
	...
	eventCh, eventSub := w.backend.WatchLatestSCRAPEEvent()
	defer eventSub.Close()

	for {
		var ev *beacon.SCRAPEEvent
		select {
		case <-w.stopCh:
			return
		case ev = <-eventCh:
		...
		scrapeState, err := w.backend.GetSCRAPEState(w.ctx, ev.Height)
		if err != nil {
			w.logger.Error("failed to query SCRAPE state",
				"err", err,
			)
			continue
		}
		...
		case beacon.StateCommit:
			w.onStateCommit(ev, scrapeState)
		case beacon.StateReveal:
			w.onStateReveal(ev, scrapeState)
		case beacon.StateComplete:
			w.onStateComplete(ev, scrapeState)
		}
	}
}

func (w *Worker) eventOkForState(ev *beacon.SCRAPEEvent, expectedState beacon.RoundState) bool {
	if w.state == nil {
		return false
	}
...
func (w *Worker) onStateCommit(ev *beacon.SCRAPEEvent, scrapeState *beacon.SCRAPEState) {
	if w.state != nil {
		if ev.Epoch == w.state.Epoch && ev.Round == w.state.Round {
	...
	h, cmpH := hash.NewFrom(points), hash.NewFrom(scrapeState.Instance.Participants)
	if !h.Equal(&cmpH) {
	...
	// Initialize the local SCRAPE instance.
	instance, err := scrape.New(&scrape.Config{
		PrivateKey:    &w.identity.BeaconScalar,
		Participants:  points,
		Threshold:     scrapeState.Instance.Threshold,
		PVSSThreshold: scrapeState.Instance.PVSSThreshold,
	})
	if err != nil {
		w.logger.Error("commit: failed to initialize SCRAPE instance",
			"err", err,
		)
		return
	}
	...
	commitPayload := beacon.SCRAPECommit{
		Epoch:  ev.Epoch,
		Round:  ev.Round,
		Commit: commit,
	}
	tx := transaction.NewTransaction(0, nil, beacon.MethodSCRAPECommit, commitPayload)
...
func (w *Worker) onStateReveal(ev *beacon.SCRAPEEvent, scrapeState *beacon.SCRAPEState) {
	var stateOk bool
	defer func() {
		if !stateOk {
			w.persistState(nil)
	...
	if scrapeState.BadParticipants[w.identity.NodeSigner.Public()] {
	...
	for i := 0; i < len(scrapeState.Participants); i++ {
		cs := scrapeState.Instance.Commits[i]
		if cs == nil {
	...
	revealPayload := beacon.SCRAPEReveal{
		Epoch:  ev.Epoch,
		Round:  ev.Round,
		Reveal: reveal,
	}
	tx := transaction.NewTransaction(0, nil, beacon.MethodSCRAPEReveal, revealPayload)
...
func (w *Worker) onStateComplete(ev *beacon.SCRAPEEvent, scrapeState *beacon.SCRAPEState) {
	...
	for i := 0; i < len(scrapeState.Participants); i++ {
		reveal := scrapeState.Instance.Reveals[i]
		if reveal == nil {
	...
	if !bytes.Equal(scrapeState.Entropy, entropy) {
		w.logger.Error("recover: BUG: entropy mismatch",
			"recovered", entropy,
			"expected", scrapeState.Entropy,
		)
	}
}
...
func (w *Worker) recoverPersistedState() {
	...
	scrapeState, err := w.backend.GetSCRAPEState(w.ctx, consensus.HeightLatest)
	if err != nil {
		w.logger.Error("restore: failed to query SCRAPE state",
			"err", err,
		)
		return
	}
	...
	if scrapeState.Epoch != state.Epoch || scrapeState.Round != state.Round {
	...
	if scrapeState.State == state.State {
		...
		switch state.State {
		case beacon.StateCommit:
			if scrapeState.Instance.Commits[participantIndex] == nil {
				tx = state.CommitTx
			}
		case beacon.StateReveal:
			if scrapeState.Instance.Reveals[participantIndex] == nil {
				tx = state.RevealTx
			}
		}
		if tx != nil {
	...
	// completed.
	if scrapeState.State != beacon.StateReveal || state.State != beacon.StateCommit {
		w.logger.Error("restore: unexpected consensus/local state for resumption",
			"state", state.State,
			"consesus_state", scrapeState.State,
		)
		w.persistState(nil)
		return
	}
...
func (w *Worker) retrySubmitTx(tx *transaction.Transaction) {
	...
	fn := func() error {
		...
		scrapeState, err := w.backend.GetSCRAPEState(ctx, consensus.HeightLatest)
		if err != nil {
			return err
		}
		...
		if scrapeState.State != expectedState {
			return backoff.Permanent(fmt.Errorf("worker/beacon: state not expected for tx: %s", scrapeState.State))
		}
...
func New(
	...
	scrapeBackend, shouldEnable := consensus.Beacon().(beacon.SCRAPEBackend)
