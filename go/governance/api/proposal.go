...
// MarshalText encodes a State into text form.
...
func (p *Proposal) CloseProposal(totalVotingStake quantity.Quantity, quorum, threshold uint8) error {
	...
	if votedStake.Cmp(&totalVotingStake) > 0 {
		return fmt.Errorf("%w: voted stake greater than total possbile voting stake", errInvalidProposalState)
	}
	...
	if voteStakePercentage.Cmp(quantity.NewFromUint64(uint64(quorum))) < 1 {
		// Reject proposal.
	...
	if votedYesPercentage.Cmp(quantity.NewFromUint64(uint64(threshold))) < 1 {
		// Reject proposal.
		p.State = StateRejected
		return nil
	}
