...
func doBeaconScenario(cmd *cobra.Command, args []string) {
	...
	backend, ok := baseBackend.(beacon.SCRAPEBackend)
	if !ok {
		panic("beacon not configured for SCRAPE backend")
	}

	// Start watching for SCRAPE events
	ch, sub := backend.WatchLatestSCRAPEEvent()
	defer sub.Close()
...
commitWaitLoop:
	...
	instance, err := scrape.New(&scrape.Config{
		...
		PVSSThreshold: state.Instance.PVSSThreshold,
	})
	if err != nil {
		return fmt.Errorf("failed to initialize SCRAPE instance: %w", err)
	}
...
func waitForBeaconState(
	...
	for {
		...
		state, err := backend.GetSCRAPEState(ctx, ev.Height)
		if err != nil {
			return nil, fmt.Errorf("failed to query SCRAPE state: %w", err)
		}
