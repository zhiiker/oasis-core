...
var (
	...
	//
	// Key format is: 0x82 <closes-at (beacon.EpochTime)> <proposal-id (uint64)>.
	// Value is activeProposalValue.
	activeProposalsKeyFmt = keyformat.New(0x82, epochtime.EpochTime(0), uint64(0))
	activeProposalValue   = []byte{1}
	...
	//
	// Key format is: 0x84 <upgrade-epoch (beacon.EpochTime)> <proposal-id (uint64)>.
	// Value is pendingUpgradesValue.
	pendingUpgradesKeyFmt = keyformat.New(0x84, epochtime.EpochTime(0), uint64(0))
	// pendingUpgradesValue  = []byte{1}
...
func (s *ImmutableState) ActiveProposals(ctx context.Context) ([]*governance.Proposal, error) {
	...
	for it.Seek(activeProposalsKeyFmt.Encode()); it.Valid(); it.Next() {
		...
		var proposalID uint64
		// || !bytes.Equal(it.Value(), activeProposalValue)
...
func (s *ImmutableState) isProposalPendingUpgrade(ctx context.Context, proposal *governance.Proposal) (bool, error) {
	...
	data, err := s.is.Get(ctx, pendingUpgradesKeyFmt.Encode(proposal.Content.Upgrade.Epoch, proposal.ID))
	return data != nil, err
}
...
func (s *ImmutableState) PendingUpgrades(ctx context.Context) ([]*upgrade.Descriptor, error) {
	...
	for it.Seek(pendingUpgradesKeyFmt.Encode()); it.Valid(); it.Next() {
		var epoch epochtime.EpochTime
		var proposalID uint64
		// || !bytes.Equal(it.Value(), pendingUpgradeValue)
		...
		if proposal.Content.Upgrade == nil {
			...
			return nil, fmt.Errorf("tendermint/governance: pending upgrade with missing upgrade descriptor")
		}
...
func (s *MutableState) SetActiveProposal(ctx context.Context, proposal *governance.Proposal) error {
	...
	err := s.ms.Insert(ctx, activeProposalsKeyFmt.Encode(proposal.ClosesAt, proposal.ID), activeProposalValue)
	return api.UnavailableStateError(err)
}
...
func (s *MutableState) RemoveActiveProposal(ctx context.Context, proposal *governance.Proposal) error {
	...
	err := s.ms.Remove(ctx, activeProposalsKeyFmt.Encode(proposal.ClosesAt, proposal.ID))
	return api.UnavailableStateError(err)
}
...
func (s *MutableState) SetPendingUpgrade(ctx context.Context, proposalID uint64, upgrade *upgrade.Descriptor) error {
	...
	err := s.ms.Insert(ctx, pendingUpgradesKeyFmt.Encode(upgrade.Epoch, proposalID), cbor.Marshal(upgrade))
	return api.UnavailableStateError(err)
}
...
func (s *MutableState) RemovePendingUpgrade(ctx context.Context, epoch epochtime.EpochTime, proposalID uint64) error {
	// Remove proposal from the active proposals list.
	err := s.ms.Remove(ctx, pendingUpgradesKeyFmt.Encode(epoch, proposalID))
	return api.UnavailableStateError(err)
}
...
func (s *MutableState) RemovePendingUpgradesForEpoch(ctx context.Context, epoch epochtime.EpochTime) error {
	...
	for it.Seek(pendingUpgradesKeyFmt.Encode(epoch)); it.Valid(); it.Next() {
		var epocht epochtime.EpochTime
		var proposalID uint64
		// || !bytes.Equal(it.Value(), pendingUpgradeValue)
	...
	for _, proposalID := range upgradeProposalIDs {
		if err := s.ms.Remove(ctx, pendingUpgradesKeyFmt.Encode(epoch, proposalID)); err != nil {
			return api.UnavailableStateError(err)
		}
	}
