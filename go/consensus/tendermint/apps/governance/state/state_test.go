...
func TestVotes(t *testing.T) {
	...
	expectedVoteEntries := []*governance.VoteEntry{
	...
	// Query votes.
	votes, err := s.Votes(ctx, proposals[0].ID)
	require.NoError(err, "Votes()")
	require.ElementsMatch(votes, expectedVoteEntries, "Vote entries should match")

	// Override vote.
	err = s.SetVote(ctx, proposals[0].ID, voterAddrs[2], governance.VoteYes)
	require.NoError(err, "SetVote()")
	expectedVoteEntries[2] = &governance.VoteEntry{Voter: voterAddrs[2], Vote: governance.VoteYes}

	// Query votes.
	votes, err = s.Votes(ctx, proposals[0].ID)
	require.NoError(err, "Votes()")
	require.ElementsMatch(votes, expectedVoteEntries, "Vote entries should match after update")
}
